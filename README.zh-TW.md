...
- 當前環境尚未完成 Jira 寫入設定時，自動降級為手動可貼的草稿格式
...
這個專案應保持開源安全與通用性。
不要放入私有 Jira 設定、真實 accountId 對應、可識別組織結構的流程規則，以及真實客戶或正式環境案例。
...
- 腳本路徑：`python3`（這個專案不會自動安裝）、Jira API token、私有設定檔
...
- 若要直接建 Jira，仍需另外完成 Jira MCP 或腳本連線設定。
...
| 情況 | skill 的行為 |
|------|------------|
| 尚未完成 Jira 設定 | 產出草稿，提示手動貼進 Jira |
| 有設定檔但 Jira MCP 沒連 | 改用腳本建單 |
| 腳本也沒設定 | 輸出完整草稿，請使用者自行貼入 |
...
| [`tests/intake-cases.md`](tests/intake-cases.md) | 收單判斷回歸測例 |
